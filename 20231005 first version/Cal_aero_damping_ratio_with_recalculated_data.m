function [result_Main]=Cal_aero_damping_ratio_with_recalculated_data(input,varargin)
...
%% NOT RECOMMEND:using the displacement direct from the kalman filter is better , this funciton is only for debug
disp("NOT RECOMMEND:using the displacement direct from the kalman filter is better , this funciton is only for debug")
    ncycle = input.ncycle;
    t = input.t;
