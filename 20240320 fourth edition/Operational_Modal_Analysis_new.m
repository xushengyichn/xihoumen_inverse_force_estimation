% % % % % % % % % % % % The latest operational modal analysis is missing,
% this file is to reproduce the function but still in progress
...
disp(vivTable.startDate_update(VIV_sel))
...
for k1 = 1:7
    str1="mode"+num2str(k1)+"_SSI";
    str2="mode"+num2str(k1)+"_seq";
    SSI_num = vivTable.(str1)(VIV_sel);
    seq_num = vivTable.(str2)(VIV_sel);
    if SSI_num>0
        frequency_temp = w_array_stable{SSI_num}(seq_num)/2/pi;
        damping_ratio = xi_array{SSI_num}(seq_num);
        omega_temp=w_array_stable{SSI_num}(seq_num);
